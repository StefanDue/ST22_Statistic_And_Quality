...
```{r}
while (N < MaxN) {
  Pi_Random.vec <- runif(N,-1,1)
  M.df <- as.vector(data.frame(xValue = sample(Pi_Random.vec, N/2, replace = TRUE), yValue = sample(Pi_Random.vec, N/2, replace = TRUE)))
...
```{r}
Autounfall.tab <- as.table(rbind(c(100,90,190), c(8910,900,9810), c(9010,990,10000)))
...
```{r}
Kontingtab.tab <- as.table(rbind(c(1,0.9,1.9), c(89.1,9,98.1), c(90.1,9.9,100)))
...
Verunf_Angeg
Anteil_Verunf_Angeg <- (Verunf_Angeg/10000)*100
Anteil_Verunf_Angeg
```
