...
```
```{r}
plot(Würfel.df$Augenzahl, Würfel.df$Summenhfk., main = "Summenhäufigkeit", xlab = "Augenzahl", ylab = "rel. Häufigkeit", type = "l", lty = 1)
...
Stiftlänge.df <- data.frame(Länge = Länge_Intervall.vec, Klassenmitte = c(3.35,3.45,3.55,3.65,3.75), Klassenbreite = rep(0.1,5), abs.Häufigkeit = Länge_Klasse.vec, rel.Häufigkeit = Länge_Klasse.vec/length(Verteilungsvergleich.df$Stiftlaenge_cm))
...
```
```{r}
barplot(Stiftlänge.df$Dichte, names = Länge_Intervall.vec, main = "Histogramm Stiftlänge", col = "blue", space = 0, xlab = "Stiftlänge in cm", ylab = "rel.Häufigkeit", ylim = c(0,4))
```
```{r}
