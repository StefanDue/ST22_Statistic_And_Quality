...
```
```{r}
plot(c(0,2,4,6,8,10,12,14,16), c(0,Gesprächsdauer.df$Summenhfk.), main =  "Summenhäufigkeit", xlab = "Gesprächsdauer in min", ylab = "rel. Häufigkeit", type = "l", lty = 1)
...
__*Stiftlänge*__
```{r}
Länge.vec <- c(3.3,3.4,3.45,3.5,3.55,3.6,3.65,3.7,3.75,3.8)
Länge_Klasse.vec <- tabulate(cut(Verteilungsvergleich.df$Stiftlaenge_cm, breaks = Länge.vec))

Stiftlänge.df <- data.frame(Länge = Länge.vec, Klassenmitte = c(3.35,3.45,3.55,3.65,3.75))
```
